...
# = Leitura do Google Sheets com batchGet (mais eficiente) =
...
# = Configuração da API Conta Azul com retry automático =
...
# = Rate limiter manual para controlar requisições =
class RateLimiter:
...
rate_limiter = RateLimiter(max_per_second=5)  # Limite conservador de 5 req/s
...
# Reduzir workers para 3-5 threads (mais estável)
...
# = Enviar dados usando batchUpdate (MUITO MAIS RÁPIDO) =
...
# **INOVAÇÃO**: Usar batchUpdate com uma única requisição
print("📊 Enviando dados em lote único (otimizado)...")
try:
