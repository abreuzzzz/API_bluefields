...
def buscar_eventos(token, inicio, fim, centro_id=None, pagina=1):
    url = f"https://api-v2.contaazul.com/v1/financeiro/eventos-financeiros/contas-a-receber/buscar"
    headers = {
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json"
    }
...
def salvar_no_postgres(dados):
    ...
    for item in dados:
        cur.execute("""
            INSERT INTO contas_a_receber (id, status, descricao, total, data_vencimento, centro_custo_nome)
            VALUES (%s, %s, %s, %s, %s, %s)
        ...
        SET 
            ...
            WHERE contas_a_receber.centro_custo_nome IS DISTINCT FROM EXCLUDED.centro_custo_nome;
        """, (
            item.get("id"),
            item.get("status"),
            item.get("descricao"),
...
def main():
    ...
    data_inicio = datetime.date(2018, 7, 1)
    data_fim = datetime.date(2018, 10, 1)
